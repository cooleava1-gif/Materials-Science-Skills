...
- test conditions (temperature, humidity, loading rate, curing time)
- instrument parameters (resolution, scan range, magnification)
- standard numbers (GB/T, ASTM, EN, AASHTO, RILEM)
- dosage ranges or mixing procedures not in the source
- statistical significance (p-values, confidence intervals, replicate counts)

If a standard number, test condition, or instrument parameter is not in the source material, flag it as `[needs verification: ...]` rather than guessing.
...
- missing citations
- unit consistency (MPa, mm/min, cm⁻¹, °C)
...
- suggest chart types or visual layouts
...
- generate reference lists from verified sources
...
- invent test conditions, instrument parameters, or standard numbers
