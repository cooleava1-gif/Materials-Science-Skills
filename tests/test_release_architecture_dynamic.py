...
class DynamicReleaseArchitectureTest(unittest.TestCase):
    ...
    def test_only_plugin_package_exposes_plugin_descriptor(self):
        self.assertTrue((PLUGIN_ROOT / ".codex-plugin" / "plugin.json").is_file())
        self.assertFalse((REPO_ROOT / ".claude-plugin").exists())

    def test_release_notes_manual_install_preserves_shared_paths(self):
    ...
    def test_generate_narrative_help_uses_plugin_default_output(self):
        ...
        self.assertNotIn(
            "default: " + "skills/materials-writing/references/<id>-narrative.md",
            script_text,
        )
        self.assertIn(
    ...
    def test_install_docs_and_mcp_config_use_same_mcp_server_location(self):
        ...
        args = plugin_mcp_config["mcpServers"]["materials-academic-search"]["args"]
        cwd = plugin_mcp_config["mcpServers"]["materials-academic-search"]["cwd"]
        ...
        self.assertEqual(
            ["./skills/materials-citation/mcp/academic_search/server.py"], args
        )
        self.assertEqual(".", cwd)

    def test_plugin_mcp_config_resolves_from_plugin_root(self):
        plugin_mcp_config = json.loads(
            (PLUGIN_ROOT / ".mcp.json").read_text(encoding="utf-8")
        )
    ...
    def test_plugin_mcp_requirements_match_imported_runtime_dependencies(self):
        ...
        self.assertIn('cwd = "/absolute/path/to/plugins/materials-skills"', docs_text)
