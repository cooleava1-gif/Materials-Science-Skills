# Materials Science Gallery

Representative visual samples for the public Materials Science Skills package.
The gallery is intentionally compact: it shows the figure style and workflow
surface without shipping the full internal generated image corpus. Data plots
are produced by the Python/R backend; conceptual/schematic figures follow the
default R hybrid route (AI decoration assets + R vector semantics,
watermark-stripped).

## Composite Overviews

These overview boards live in this folder and are kept as lightweight public
proof of the `materials-figure` workflow:

![Chart-Type Atlas](gallery_chart_atlas.png)

![WER-EA Figures](gallery_wer_ea_workflow.png)

![Cross-Material-System Figures](gallery_material_systems.png)

## Public Asset Boundary

The public repository keeps representative samples under:

| Asset layer | Public boundary |
|---|---|
| Preview boards | The three PNG files in this `docs/gallery/` folder |
| Figure templates | Contract, package, caption, and QA templates under `materials-figure/assets/templates/` |
| Figure examples | Small runnable source examples under `materials-figure/examples/figure-packages/` |

The generated chart atlas, gallery composites, showcase boards, full visual
pack, and internal regression suite are not part of the GitHub delivery. If a
future release needs the complete visual pack, it should be published
separately from the installable skill bundle.

## Outcome Showcases

See [docs/showcases](../showcases/README.md) for the submission package,
reviewer-response, and FAIR-data routes that use these public proof assets.
