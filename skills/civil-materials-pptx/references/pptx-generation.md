...
Use `scripts/build_civil_materials_pptx.py`.

Common commands:

```bash
python scripts/build_civil_materials_pptx.py --title "Waterborne epoxy modified emulsified asphalt" --presenter "种浩然" --output deck.pptx
```

With a Markdown outline:

```bash
python scripts/build_civil_materials_pptx.py --input outline.md --output deck.pptx
```

With a JSON outline:

```bash
python scripts/build_civil_materials_pptx.py --input outline.json --output deck.pptx
```

With a preset:

```bash
python scripts/build_civil_materials_pptx.py --deck-type journal-club --language zh --title "Paper title" --output deck.pptx
```

With one command-line image and crop:

```bash
python scripts/build_civil_materials_pptx.py --title "Bonding performance" --image figure.png --image-slide 5 --crop 4,0,4,8 --output deck.pptx
```
